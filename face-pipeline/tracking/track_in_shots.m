...
det_string=[result_dir '%06d.mat'];
%parfor i = 1:size(shots, 2)
for i = 1:size(shots, 2)
    ...
    det         = [];
    for f = s1:s2
		%change a bit the command so that it doesn't violate parfor's transparency
        box=load(sprintf(det_string, f));
		box=box.box;
        for j = 1:size(box, 1)
            det(end + 1).frame = f;
